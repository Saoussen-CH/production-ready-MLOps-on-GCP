...
from pipelines.utils.trigger_pipeline_from_payload import trigger_pipeline_from_payload
...
def gcs_uri(output_uri: str, storage_client):
    """
...
def pipeline_e2e_test(
    ...
    **kwargs: dict,
    ...
    try:
        ...
        logging.error("Error triggering pipeline: %s", str(e))
    ...
    for task_name, expected_output in expected_tasks.items():
        ...
        for output_artifact in expected_output:
            ...
            # if the output uri is a gcs path, fetch the file
                ...
                file_size = gcs_uri(output_uri, storage_client)
                assert (
                    file_size > 0
